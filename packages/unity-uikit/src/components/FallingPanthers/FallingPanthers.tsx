...
import PantherIcon from "../Svg/Icons/Logo";
import { PantherProps, FallingPanthersProps } from "./types";

const pantherFall = keyframes`
  0% {
    opacity: 1;
...
const Panther = styled.div<PantherProps>`
  display: inline-flex;
  position: fixed;
  top: 0;
...
const FallingPanthers: React.FC<FallingPanthersProps> = ({
  count = 30,
  size = 32,
  iterations = Infinity,
  duration = 10,
}) => {
  const panthers = [...Array(count)].map((_, index) => (
    <Panther key={String(index)} position={Math.random() * 100} iterations={iterations} duration={duration}>
      <PantherIcon width={size} height={size} />
    </Panther>
  ));

  return <div>{panthers}</div>;
};
